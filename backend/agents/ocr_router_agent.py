...
Given a preprocessed image and a document class, dispatch to the correct OCR
agent and return a uniform :class:`OCRResult`:

    TABLE        -> TableOCRAgent        (Granite Vision 4.1-4b + fallback)
    PRINTED_TEXT -> PrintedOCRAgent      (PaddleOCR -> Tesseract)

Legacy 2-class label (``"printed"``) is accepted for backward compatibility
with stored ``doc_type`` values.
...
def run_ocr(image: np.ndarray, doc_class: str) -> OCRResult:
    ...
    :param doc_class: one of TABLE / PRINTED_TEXT (also accepts the
        legacy lower-case ``printed``).
